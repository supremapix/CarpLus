...
function purposeByCategory(categoria) {
...
function buildComparison(tire) {
  const candidates = TIRES.filter(
    (t) => t && t.slug && t.id !== tire.id && t.medida === tire.medida && t.marca !== tire.marca
  );
  const pool = candidates.length > 0 ? candidates : TIRES.filter((t) => t && t.slug && t.id !== tire.id && t.aro === tire.aro && t.marca !== tire.marca);
...
function generateTireContent(tire) {
