...
import { MessageSquare, Phone, ChevronRight, CircleCheck as CheckCircle2, ShieldCheck, Clock, Award, Star, ArrowLeft, CarFront, Share2, Copy, Check } from 'lucide-react';
import { Tire } from '@/lib/data';
import Navbar from '@/components/Navbar';
import Footer from '@/components/Footer';
import { useState } from 'react';

interface TireDetailClientProps {
  tire: Tire;
  relatedTires: Tire[];
}

export default function TireDetailClient({ tire, relatedTires }: TireDetailClientProps) {
  ...
  const shareUrl = `https://www.carpluspneuseoficina.com.br/pneu/${tire.slug}`;
    ...
    <div className="min-h-screen bg-gray-50 pt-24 md:pt-28">
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 py-8">
        ...
        <nav className="flex items-center gap-2 text-[10px] md:text-xs font-bold uppercase tracking-widest text-gray-400 mb-8 overflow-x-auto whitespace-nowrap pb-2">
            ...
            <Link href="/pneus" className="hover:text-black">Pneus</Link>
            <ChevronRight size={12} />
            <Link href={`/pneus?marca=${tire.marca.toLowerCase()}`} className="hover:text-black">{tire.marca}</Link>
            <ChevronRight size={12} />
        ...
        {relatedTires.length > 0 && (
            <section className="mb-20 px-4">
                <div className="flex items-center justify-between mb-12">
                    ...
                    <Link href="/pneus" className="text-xs font-bold uppercase tracking-widest hover:text-primary transition-colors flex items-center gap-2">
