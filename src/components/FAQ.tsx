...
const FAQS = [
  { q: "Onde fica a Carplus Pneus e Oficina Mecânica em Curitiba?", a: "Estamos na Av. Arthur da Silva Bernardes, 1323 – Portão, Curitiba – PR. Referência: próximo ao Shopping Palladium. Fácil acesso pelas avenidas República Argentina, Sete de Setembro e Winston Churchill." },
  ...
  { q: "Preciso agendar ou posso ir direto?", a: "Pode vir sem agendamento! Para serviços mais complexos como revisão completa ou suspensão, recomendamos ligar antes: (41) 3082-7282. Horário: Seg–Sex 8h–18h | Sáb 8h–12h." },
...
function FAQItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className="border-b border-gray-100 last:border-none">
      ...
      >
        <span className={`text-sm md:text-xl font-bold transition-colors pr-4 whitespace-normal break-words overflow-wrap-anywhere flex-1 ${isOpen ? 'text-primary' : 'text-dark hover:text-primary/70'}`}>{q}</span>
        <motion.div
          animate={{ rotate: isOpen ? 180 : 0 }}
          className={`p-2 rounded-full flex-shrink-0 self-start ${isOpen ? 'bg-primary text-white' : 'bg-gray-100 text-gray-400 group-hover:bg-gray-200'}`}
        >
          <ChevronDown size={20} />
        </motion.div>
      </button>
  ...
  return (
    <section className="py-24 bg-white overflow-hidden">
      <div className="max-w-4xl mx-auto px-4">
        ...
        <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl mb-12 font-bold">Perguntas <span className="text-primary italic">Comuns</span></h2>
